...
var numberRegExp = new RegExp(/-?\d+(?:\.\d+)?/, "g");
function extractProperty(value) {
    var valueNumList = (value.match(numberRegExp) || []).map(function (n) { return Number(n); });
    var valueStrList = value.split(numberRegExp);
    return {
...
var Scroll = (function () {
    ...
    Scroll.prototype.initItemsEffects = function () {
        this.stages.forEach(function (stage) {
            walkNode(stage.node, function (node) {
                ...
                if (itemId) {
                    var items = stage.stageConfig.items;
                    for (var i = 0; i < items.length; i++) {
                        if (items[i].id === itemId) {
                            (stage.items || (stage.items = [])) && stage.items.push(__assign(__assign({}, items[i]), { node: node }));
                            items[i].effects.forEach(function (effect) {
                                node.style[effect.property] = effect.start;
                            });
                        }
                    }
                }
            ...
            stage.stageConfig.items.forEach(function (item) {
                var effects = item.effects;
                effects.forEach(function (effect) {
                    if (effect.startAt == undefined)
                        effect.startAt = 0;
                    if (effect.endAt == undefined)
                        effect.endAt = Number(stage.stageConfig.scrollNumber);
                    ["start", "end"].forEach(function (key) {
                        var value = effect[key];
                        try {
                            var color = Color(value).rgb().toString();
                            effect[key] = color;
                        }
                        catch (e) {
                        }
                        finally {
                            var _a = extractProperty(effect[key]), valueNumList = _a.valueNumList, valueStrList = _a.valueStrList;
                            var numSignature = (key + "Numbers");
    ...
    Scroll.prototype.activateStage = function () {
        var activeStage = this.stages[this.activeStageIndex];
        var stageProxy = new Proxy(this, {
            ...
            set: function (obj, prop, value) {
                if (prop === "activeStage") {
                    if (value === activeStage)
                        return;
                    var preStage = JSON.parse(JSON.stringify(activeStage));
                    activeStage = value;
                    obj.activeStageIndex = obj.stages.findIndex(function (stage) { return stage === value; });
                    obj.handleStageChange();
                    obj.target.dispatchEvent(new CustomEvent("stage-change", {
                        detail: {
                            previous: {
                                id: preStage.id,
                                node: preStage.node,
    ...
    Scroll.prototype.handleStageChange = function () {
        var _this = this;
        clearTimeout(this.switchTimeout);
        this.switching = true;
        this.stages.forEach(function (stage) {
            stage.node.style["transform"] = "translateY(".concat(-_this.activeStageIndex * 100, "%)");
        });
    ...
    Scroll.prototype.addEventListeners = function () {
        this.target.addEventListener("wheel", this.handleScroll.bind(this));
    };
    Scroll.prototype.handleScroll = function (event) {
        event.preventDefault();
        if (this.animating || this.switching)
            return;
        this.stagesProxy.activeStage.step += event.deltaY > 0 ? 1 : -1;
        this.handleStepChange();
    };
    Scroll.prototype.hasNextStage = function () {
        return true;
    };
    Scroll.prototype.handleStepChange = function () {
        var _this = this;
        var curStage = this.stagesProxy.activeStage;
        var step = curStage.step;
        var stageConfig = curStage.stageConfig;
        var items = curStage.items;
        var curIndex = this.activeStageIndex;
        if (step > Number(stageConfig.scrollNumber)) {
            if (curIndex === this.stages.length - 1) {
                console.log(1);
                this.target.dispatchEvent(new CustomEvent("scroll-end", {
                    detail: {}
                }));
                curStage.step -= 1;
                return;
            ...
            items.forEach(function (item) {
                item.node.style.transition = "".concat(stageConfig.transition, "ms ").concat(stageConfig.easing);
                item.effects.forEach(function (effect) {
                    item.node.style[effect.property] = Scroll.getNextStyleValue(effect, step);
                });
            });
    ...
    Scroll.prototype.setActiveStage = function (id, changeByScroll) {
        if (changeByScroll === void 0) { changeByScroll = false; }
        if (this.stagesProxy.activeStage.id === id)
            return;
        var oldIndex = this.activeStageIndex;
        ...
        var newIndex = this.stages.findIndex(function (stage) { return stage === newStage; });
        if (changeByScroll) {
    ...
    Scroll.getNextStyleValue = function (effect, step) {
        var startAt = effect.startAt, endAt = effect.endAt, startNumbers = effect.startNumbers, endNumbers = effect.endNumbers, strings = effect.strings, isColor = effect.isColor;
        step = Math.min(endAt, Math.max(startAt, step));
        var result = strings[0];
        var alphaIndex = -1;
        if (startNumbers && startNumbers.length > 0) {
            startNumbers.forEach(function (startNumber, index) {
                if ((/rgb/).test(strings[index]))
                    alphaIndex = index + 3;
                var stepNumber = startNumber + (step - startAt) * (endNumbers[index] - startNumber) / (endAt - startAt);
                if (isColor && index !== alphaIndex)
                    stepNumber = Math.round(stepNumber);
                result += "".concat(stepNumber).concat(strings[index + 1]);
            });
        }
        return result;
    };
